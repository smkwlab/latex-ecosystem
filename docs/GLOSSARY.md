...
本エコシステムの文書で使う中核概念の用語を定義します。**1 つの概念には 1 つの名称**を使います。用語を追加・変更するときは、まず本ファイルを更新し、末尾の手順で各文書へ伝播させてください。
...
- **備考**: 学生に伝わりやすい名称は試行錯誤中であり、本語は現時点の採用語。旧呼称「draft-to-draft PR workflow」「draft-chain」「ドラフトレビューワークフロー」は [#137](https://github.com/smkwlab/latex-ecosystem/issues/137) で本語に統一した
...
「Pull Request ベース添削」⊃「draft PR サイクル」の関係にあり、両者は同じものの言い換えではなく**異なる範囲を指す別の用語**である（学生は中核のサイクルだけ理解すれば執筆でき、教員向け文書は全体を扱う、という役割分担）。
