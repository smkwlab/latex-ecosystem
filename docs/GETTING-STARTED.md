...
```bash
brew install gh        # macOS（Ubuntu: sudo apt install gh / Windows: scoop install gh）
gh auth login          # 認証
gh auth status         # 認証の確認
```
...
```bash
mkdir latex-ecosystem-dev && cd latex-ecosystem-dev
gh repo clone smkwlab/latex-ecosystem .   # または git clone https://github.com/smkwlab/latex-ecosystem.git .
./setup.sh    # 全コンポーネントリポジトリ（基盤・テンプレート・ツール）を clone
```
