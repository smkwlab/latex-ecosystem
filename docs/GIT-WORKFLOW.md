...
ローカル `main` に直接 commit し、リモートで PR がマージされる間にローカル `main` を `origin/main` から乖離させると、避けられたはずのマージコンフリクトが生じます。
以下のルールは、すべての変更を feature ブランチに載せ、`main` をリモートのクリーンなミラーに保つことで、`git pull` を常に fast-forward にします。
