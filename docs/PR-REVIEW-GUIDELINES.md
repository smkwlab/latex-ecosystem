...
学生向け文書（STUDENT-WORKFLOW.md・各テンプレート README）では、この添削フローの中核である「draft ブランチで執筆 → PR で添削 → 自動作成される次稿ブランチで改稿」の繰り返しを **draft PR サイクル**と呼びます。読者層ごとの呼称の対応は [GLOSSARY.md](GLOSSARY.md) を参照してください。
