defmodule EcosystemManager.GitHubTest do
  ...
  describe "GitHub API統合テスト" do
    test "handles API rate limiting gracefully" do
      ...
      System.cmd("git", ["remote", "add", "origin", "https://github.com/test/repo.git"], cd: test_repo_path)
    ...
    test "handles concurrent API calls safely" do
      ...
      repos = for i <- 1..5 do
        ...
        System.cmd("git", ["init"], cd: test_repo_path)
        System.cmd("git", ["remote", "add", "origin", "https://github.com/test/repo#{i}.git"], cd: test_repo_path)
        
        %Repository{name: "repo#{i}", path: test_repo_path, status: :ok}
      end
      
      # Test concurrent API calls
      tasks = for repo <- repos do
  ...
  describe "エラーハンドリングの強化" do
    ...
    test "handles invalid JSON response" do
      ...
      System.cmd("git", ["init"], cd: test_repo_path)
      System.cmd("git", ["remote", "add", "origin", "https://github.com/nonexistent/repo123456789.git"], cd: test_repo_path)
    ...
    test "handles authentication failures" do
      ...
      System.cmd("git", ["init"], cd: test_repo_path)
      System.cmd("git", ["remote", "add", "origin", "https://github.com/private/repo.git"], cd: test_repo_path)
