...
- **elixir-tool-kit**: 上記 3 つの Elixir CLI ツールが共有する基盤ライブラリ
  (CLI spec エンジン・設定レイヤ・出力レンダリング・GitHub クライアント・
  ファイルキャッシュ)。git 依存の semver タグ固定で参照する
...
3 つの Elixir CLI ツールは 1 つの設定スキームを共有する(2026-07 決定;
smkwlab/thesis-monitor#14/#16/#18/#20、registry-manager#16/#18/#21、
ecosystem-manager#10)。レジストリ関連キーの意味論は 2 つのレジストリツール
(registry-manager / thesis-monitor)のものである:
...
3 つの Elixir CLI ツール(registry-manager / thesis-monitor /
ecosystem-manager)は共通基盤ライブラリ **smkwlab/elixir-tool-kit** の CLI
エンジンの上で 1 つのコマンド体系を共有する(2026-07 決定;
latex-ecosystem#146):

- **宣言的 spec が単一情報源**: 各ツールはオプションカタログとコマンド表を
  自リポジトリの `<Tool>.CLI.Spec` に持ち、strict パース・コマンド別
  オプション検証・enum 検証・help 生成(グローバルと `<command> --help`)は
  `ToolKit.CLI.{Spec,Parser,Exit}` が spec から導出する。spec に無い
  オプションはパース段階でエラーになる(黙殺しない)
- **exit code**: 成功 0 / エラー 1。テストからは `test_mode` で
  `throw({:cli_test_exit, code})` として検証する
- **短フラグの正準語彙**: `-h`=help、`-v`=verbose、`-c`=config、`-f` は
  force に予約(`--format` と `--fast` は long のみ)、`-t`=時刻ソート、
  `-r`=逆順、`-l`=詳細表示。全語彙表は
  [docs/TOOL-CLI-CONVENTIONS.md](docs/TOOL-CLI-CONVENTIONS.md) を参照
- **機構はライブラリ、方針はツール**: elixir-tool-kit は機構(パース・
  4 層設定マージ・表示幅対応テーブル・GitHub REST ラッパ・TTL ファイル
  キャッシュ)を提供し、コマンド語彙・設定スキーマ・ドメインロジックは
  各ツールが持つ
- **依存の張り方**: git 依存の semver タグ固定
  (`{:tool_kit, github: "smkwlab/elixir-tool-kit", tag: "vX.Y.Z"}`)。
  採用側 PR が明示的に pin を上げ、mix.lock の commit hash が再現性を担保する
