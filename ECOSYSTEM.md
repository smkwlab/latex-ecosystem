...
- 中核概念の呼称はエコシステム内の文書では「draft PR サイクル」(英語文書では draft PR cycle)で統一する。教員向けの上位概念「Pull Request ベース添削」と論文でのシステム非依存表現のみ別扱い。対応表は [docs/GLOSSARY.md](docs/GLOSSARY.md) を正とする
- 破壊的変更には移行ガイドを含める
